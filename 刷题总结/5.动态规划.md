...
- DP问题的时间复杂度 = 状态数量 * 每一个状态计算所需的时间
- **当状态转移方程中含有(i - 1)时，初始化都从1开始(与第二条相对应)**
- **动态规划问题一般从最后一步划分问题子集**
- **动态规划问题初始化时一般为了应对`i - 1`，都从1开始读入数据；如果遇上lc题目，要从1开始循环，读取所给数组时记得向前偏移**
...
- `f(i, j) = k`表示在条件约束(i, j)下所有选取方法的最大值为k(背包问题一般有两个约束条件，因此是二维动态规划问题；状态最终表示的是代表集合的一个属性)，因此最终应该输出`f(N,V)`
- 状态表示：用几维变量表示状态，每一维的含义分别表示什么约束？
- 状态计算：如何把每一个状态划分为更小的子集进而递推计算出来？
...
int main()
{
    ...
    for (int i = 1; i <= n; i ++) //外层循环从i=1开始，因为i=0表示从前0件物品中选择没有实际意义
        for (int j = 0; j <= m; j ++)
        {
            ...
            if (j >= v[i]) //只有当背包的总体积【不小于】第i个物品的体积时，右边子集才有可能不为空
...
- 状态转移方程为：`f[i,j] = f[i - 1, j - k * v[i]] + k * w[i]`($k$为从0到背包中全部装第$i$类物品的数量，因为有体积限制所以不可能无限装)

> 优化思路：枚举状态转移方程找规律

![](./图片5/完全背包问题2.png)

第二行左边`f[i][j - v]`表示从前$i$个物品中选择体积不超过$j - v$的物品的所有选法，因此若不选第$i$个物品，集合表示为`f[i -1][j - v]`；若选择$k$个第$i$种物品，集合表示为`f[i - 1][j - (k + 1) * v] + k * w`
...
- 依次将1023个物品打包为十组，将每组看做01背包问题中的一个物品，那么通过这十个不同物品的01选用，就可以凑出0-1023个物品的取值。读取时相当于要对$i$类物品都要进行打包
...
- 动态规划：输入的二维数组`boxTypes.size()`表示的是有几种物品，`boxTypes[i][0]`表示第$i$种物品的数量，`boxTypes[i][1]`表示第$i$种物品的价值，而每种物品占用的体积均为1，`truckSize`表示卡车的体积，这样可以转化为完全背包问题，但是用朴素做法超时，二进制优化思路还没尝试
- 贪心：只能装`truckSize`个箱子到卡车上，根据贪心的思路，只需要每次都拿当前剩下的箱子里单元数量最大的箱子即可。对`boxTypes`按照 `numberOfUnitsPerBox`进行逆序排序，然后从左至右填充`truckSize`即可。**这是此题的最佳解法**
...
- 与多重背包问题相区别，多重背包问题的划分思路是对于第$i$类物品选几个
...
- 初始条件：`dp[2] = 1`，因为`2 = 1 + 1`，但要注意初始化`dp[0]`和`dp[1]`是没有意义的
...
- `dp[i]`表示以`nums[i]`为结尾的子数组元素的最大和，然后再在所有最大和中取最大值即为子数组的最大和
![](./图片5/最大子数组和.png)
- 注意和一般的动态规划问题不一样，还需要再取一次最大值，每一次遍历只能得到以当前元素结尾的子数组的和最大值，再取全局最大值才能得到最终结果；只需要记录以上一个元素结尾的子数组的最大和，因此可以实现滚动优化，空间复杂度为O(1)

:rocket:  **[LeetCode 121. 买卖股票的最佳时机](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock/)** 
- 贪心：遍历所给数组，记录遍历过的元素的最小值，用当前值减去最小值即可得到当天卖出可以获得的最大利润(也就是不需要关心是那一天买入的)，最后取全局最大值即可
- 注意和上一题的区别是如果没有利润，可以返回0，所以将`res`初始化为0

:rocket:  **[LeetCode 122. 买卖股票的最佳时机 II](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-ii/description/)** 
- 贪心：只要今天的价格比昨天的高，就在昨天买入今天卖出，这样可以保证利润最大化。因为题目允许在当天买入当天卖出，后面如果价格再升高，相当于可与在今天再买入，也就是不限制交易次数

:rocket:  **[LeetCode 123. 买卖股票的最佳时机 III](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-iii/description/)** 
![](./图片5/买卖股票的最佳时机3.png)
- 通过枚举前后两次交易的分界点，这样就分解成了两个独立的子问题，最后取两段各自的最大值相加即可
- 对于第一次交易，`f[i]`表示在前$i$天进行交易的最大利润，如果恰好就在第$i$天卖出，利润为`prices[i] - minp`；如果在第$i$天没有交易，利润为`f[i - 1]`。时间复杂度为O(n)
- 对于第二次交易，逆序遍历数组记录价格最高点，与当前价格作差取最值即可得到最大利润。时间复杂度为O(n)
- 第二次枚举时，可以顺便枚举两次交易的分界点，将两次循环合并为一次

:rocket:  **[LeetCode 188. 买卖股票的最佳时机 IV](https://leetcode.cn/problems/best-time-to-buy-and-sell-stock-iv/description/)** 
![](./图片5/买卖股票的最佳时机4.png)
- 如果允许交易的次数 ≥ 可以交易天数的一半，那么就相当于不限制交易次数，因此可以直接使用贪心算法(LC 122)
- 边的数量表示进行到可交易的第几天，圈的数量表示完成的交易笔数，因此可以将本题转化为一个二维的动态规划问题，抽象为状态机模型
...
int main()
{
    ...
    int res = -INF; // 必须初始化为-INF，因为要求的是路径和，有可能出现负数
    // 遍历数字三角形的底层，因为其中的任何一个点都可能是最大值路径的终点
    for (int i = 1; i <= n; i ++)
        res = max(res, f[n][i]);
...
- 因为`f[i]`表示的集合是所有以第$i$个数结尾的上升子序列，因此该序列中一定含有第$i$个数，所以**状态划分的依据是所求的上升子序列中倒数第二个数是原序列中的第几个数**(第0个数代表上升子序列中只有第$i$个数，此时上升子序列的长度为1)，
...
- 对于长度相同的上升子序列，只存储末尾数最小的序列。在这种情况下的长度不同的上升子序列，可以证明：长度越长，子序列末尾的数越大。记当前保存的长度为5的上升子序列的末尾数为`a`，当前保存的长度为6的上升子序列的末尾数为`b`，如果`a >= b`，那么长度为5的上升子序列的末尾数应该为长度为6的上升子序列的倒数第二个数，而不是`a`(因为第一步操作过程存储的是末尾的数最小的序列)，矛盾
- 因此对于原序列中某个特定的数，只需要在存储上升子序列末尾最小的数的数组中，用**整数二分法**找到比该数小且与该数差值最小的数，再通过以这个数结尾的最长上升子序列长度加一即可得到问题的答案
...
int n;
int a[N];
int q[N]; //q[i]为长度为i的上升子序列中末尾的最小数
...
int main()
{
    ...
    q[0] = -2e9; // 为了保证数组中最小的数一定存在
    for (int i = 0; i < n; i ++)
    {
        int l = 0, r = len;
        while (l < r) // 找到(l, r)之间小于a[i]且与a[i]差值最小的数
        {
            int mid = l + r + 1 >> 1;
            if (q[mid] < a[i])
                l = mid;
        ...
        len = max(len, r + 1);
        q[r + 1] = a[i]; //此时a[i]为长度(r + 1)的上升子序列中最小的末尾数
    }
...
- `f[i - 1][j]`表示的集合是所有在第一个字符序列的前$i-1$个字母中出现，且在第二个字符序列的前$j$个字母中出现的子序列，但其中并不一定包含`b[j]`，所以包含`f[i - 1][j]`表示的集合包含01方案，同理`f[i][j - 1]`表示的集合包含10方案
...
int main()
{
    ...
    // 动态规划问题注意从下标1开始读取，为了应对后面递推公式中的i - 1
    for (int i = 1; i <= n; i ++)
        cin >> a[i];
...
:rocket:  **[LeetCode 300. 最长递增子序列](https://leetcode.cn/problems/longest-increasing-subsequence/description/)**

:rocket:  **[LeetCode 1143. 最长公共子序列](https://leetcode.cn/problems/longest-common-subsequence/description/)**
...
int main()
{
    ...
    for (int i = 0; i < n; i ++)
        scanf("%s", str[i] + 1); // 善用scanf的这种读入方式
...
- 状态表示时表示的是某一个区间，并且循环时按照区间的长度与起点进行循环
