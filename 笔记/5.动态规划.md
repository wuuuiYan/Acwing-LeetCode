...
- 状态转移方程为：`f[i,j] = f[i - 1, j - k * v[i]] + k * w[i]`(k从0到背包中全部装第i个物品的数量，因为有体积限制所以不可能无限装)
...
- 状态转移方程为：`f[i,j] = f[i - 1, j - k * v[i]] + k * w[i](k从0到s[i])`
...
> 二进制优化：依次将1023个物品打包为十组，将每组看做01背包问题中的一个物品，那么通过这十个不同物品的01选用，就可以凑出0-1023个物品的取值
...
- 状态的计算依次可以分为：不从第`i`组中选、选择第`i`组中第1个物品、选择第`i`组中第2个物品、...、选择第`i`组中第k个物品(0 < k < s[i])
